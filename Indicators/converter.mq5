...
//+------------------------------------------------------------------+
//|                                                    bbsqueeze.mq4 |
//|                Copyright ｩ 2005, Nick Bilak, beluck[AT]gmail.com |
//+------------------------------------------------------------------+
...
//----
   ...
   limit=Bars(_Symbol, PERIOD_CURRENT)-31;
   if(counted_bars>=31) limit=Bars(_Symbol, PERIOD_CURRENT)-counted_bars-1;
   for(shift=limit;shift>=0;shift--)
     {
      //d=MQL4iMomentum(NULL,0,momPrd,PRICE_CLOSE,shift);
      ...
      diff=MQL4iATR(NULL,0,keltPrd,shift)*keltFactor;
      std=MQL4iStdDev(NULL,0,bolPrd,MODE_SMA,0,PRICE_CLOSE,shift);
      bbs=bolDev * std/diff;
      if(bbs<1)
        {
         upK[shift]=0;
   ...
   for(int x=0; x<=Len-1;x++)
     {
      ...
      Sum1+=x* (Close[x+shift]-((HH+LL)/2 + MQL4iMA(NULL,0,Len,0,MODE_EMA,PRICE_CLOSE,x+shift))/2);
      SumY+=(Close[x+shift]-((HH+LL)/2 + MQL4iMA(NULL,0,Len,0,MODE_EMA,PRICE_CLOSE,x+shift))/2);
     }
   Sum2=SumBars * SumY;
